...
static void codegen_write_reg_or_mem(Token t, u8 num){
    switch(t.type){
        case TOKEN_adc:
            compiler_write_byte(0x88 | num);
            break;
        case TOKEN_add:
            compiler_write_byte(0x80 | num);
            break;
        case TOKEN_ana:
            compiler_write_byte(0xA0 | num);
            break;
        case TOKEN_cmp:
            compiler_write_byte(0xB8 | num);
            break;
        case TOKEN_dcr:
            compiler_write_byte(0x05 | (num << 3));
            break;
        case TOKEN_inr:
            compiler_write_byte(0x04 | (num << 3));
            break;
        case TOKEN_mvi:
            compiler_write_byte(0x06 | (num << 3));
            break;
        case TOKEN_ora:
            compiler_write_byte(0xB0 | num);
            break;
        case TOKEN_sbb:
            compiler_write_byte(0x98 | num);
            break;
        case TOKEN_sub:
            compiler_write_byte(0x90 | num);
            break;
        case TOKEN_xra:
            compiler_write_byte(0xA8 | num);
            break;
        default:
...
static void codegen_write_regpair(Token t, u8 reg){
    ...
    switch(t.type){
        case TOKEN_dad:
            compiler_write_byte(0x09 | (reg << 4));
            break;
        case TOKEN_dcx:
            compiler_write_byte(0x0B | (reg << 4));
            break;
        case TOKEN_inx:
            compiler_write_byte(0x03 | (reg << 4));
            break;
        case TOKEN_ldax:
            compiler_write_byte(0x0A | (reg << 4));
            break;
        case TOKEN_lxi:
            compiler_write_byte(0x01 | (reg << 4));
            break;
        case TOKEN_pop:
            compiler_write_byte(0xC1 | (reg << 4));
            break;
        case TOKEN_push:
            compiler_write_byte(0xC5 | (reg << 4));
            break;
        case TOKEN_stax:
            compiler_write_byte(0x02 | (reg << 4));
            break;
        default:
