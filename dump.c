...
static void posix_signal_handler(int sig, siginfo_t *siginfo, void *context) {
	...
	printf("\n");
	switch(sig) {
		case SIGSEGV: perr("Caught SIGSEGV: Segmentation Fault!"); break;
		case SIGINT:
			perr("Caught SIGINT: Interactive attention signal (usually "
			     "ctrl+c)!");
			break;
		case SIGFPE:
			switch(siginfo->si_code) {
				...
				case FPE_FLTSUB:
					...
					break;
				default: perr("Caught SIGFPE: Arithmetic Exception!"); break;
			}
		case SIGILL:
			switch(siginfo->si_code) {
				case ILL_ILLOPC: perr("Caught SIGILL: Illegal opcode!"); break;
				case ILL_ILLOPN: perr("Caught SIGILL: Illegal operand!"); break;
				case ILL_ILLADR:
					perr("Caught SIGILL: Illegal addressing mode!");
					break;
				case ILL_ILLTRP: perr("Caught SIGILL: Illegal trap!"); break;
				case ILL_PRVOPC:
				...
				case ILL_BADSTK:
					...
					break;
				default: perr("Caught SIGILL: Illegal Instruction!"); break;
			}
			break;
		case SIGTERM:
			perr("Caught SIGTERM: A termination request was sent to the "
			     "program!");
			break;
		case SIGABRT:
...
static void    set_signal_handler() {
    /* setup alternate stack */
    {
        ...
        if(sigaltstack(&ss, NULL) != 0) {
            perr("Failed to set alternate sigaltstack!");
        }
    }
...
#define REG_HAND(x)                          \
	...
	perr("Unable to register handler for " #x "!")
