...
static void print_reg(u8 *memory, u16 *pointer) {
	...
	switch(memory[*pointer]) {
		case REG_A: reg = 'a'; break;
		case REG_B: reg = 'b'; break;
		case REG_C: reg = 'c'; break;
		case REG_D: reg = 'd'; break;
		case REG_E: reg = 'e'; break;
		case REG_H: reg = 'h'; break;
		case REG_L: reg = 'l'; break;
	}
	if(reg == 'X')
...
#define GET_PAIR(num) (num == REG_B ? "BC" : num == REG_D ? "DE" : "HL")
	...
	switch(code) {
		...
		case BYTECODE_adc_M:
		case BYTECODE_sbb_M:
			printf("\t(HL : 0x%x, memory[0x%x] : 0x%x)", FROM_HL(), FROM_HL(),
			       memory[FROM_HL()]);
			break;
		case BYTECODE_ldax:
		case BYTECODE_stax: {
			u16 addr = FROM_PAIR(memory[pointer - 1], memory[pointer - 1] + 1);
			printf("\t(%s : 0x%x, memory[0x%x] : 0x%x)",
			       GET_PAIR(memory[pointer - 1]), addr, addr, memory[addr]);
			break;
		}
		case BYTECODE_pop_PSW:
		case BYTECODE_push_PSW: {
			printf("\t(A : 0x%x, FLAGS : 0x%x)", m->registers[REG_A],
			       m->registers[REG_FL]);
			break;
		}
		default: break;
